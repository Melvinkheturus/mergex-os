...
export function OpportunityDetailClient({ opportunityId }: { opportunityId: string }) {
  ...
  const checkDiscovery = meetings.some(m => m.status === "COMPLETED");
                ...
                return (
                  <div key={s.id} className="flex items-center gap-1 flex-1 min-w-0 justify-center">
                    ...
                    >
                      ...
                      <span className="truncate max-w-[80px] sm:max-w-none">{s.label.split(" ").slice(1).join(" ")}</span>
                    </button>
                    ...
                    {meetings.length === 0 ? (
                      <div className="space-y-3 pt-2">
                        ...
                        <div className="p-3 border border-violet-500/10 bg-violet-500/5 rounded-xl space-y-2 pt-3">
                          <p className="text-[10px] font-bold uppercase tracking-widest text-[#8B5CF6]">Schedule First Discovery Meeting</p>
                      ...
                      <div className="space-y-2">
                        {meetings.map((m) => (
                          <div key={m.id} className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 p-3 rounded-lg border border-border/20 bg-background/50 text-xs">
                            ...
                            <Badge className={cn(
                              "text-[9px] uppercase font-bold px-2 self-start sm:self-center border",
                              m.status === "COMPLETED"
                                ? "bg-emerald-500/10 text-emerald-500 border-emerald-500/20"
                                : m.status === "CANCELLED"
                                ? "bg-rose-500/10 text-rose-500 border-rose-500/20"
